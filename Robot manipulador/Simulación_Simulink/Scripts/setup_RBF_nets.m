function NN = setup_RBF_nets(ndof,kmeans_data)
    %kmeans_data = q';%r';
    ...
    elseif ndof == 6
        [num_inputs,dim_inputs,delta,weights] = RBFparams6dof(k,ndof);
    end

    NN = NN_RBF(num_inputs,dim_inputs,delta,weights,k_params);
end

function [num_inputs,dim_inputs,delta,weights] = RBFparams6dof(k,ndof)
    num_inputs = 4; % 1 = e_q, 4 = [e_q,e_px,e_py,e_pz]
    dim_inputs = ndof;

    deltaWc = 1;%1e-3;
    weightsWc = 30*ones(1,k*dim_inputs);
    ...
    deltaPc = 1;%1e-3;
    weightsPc = 30*ones(1,k*dim_inputs);
...
function [num_inputs,dim_inputs,delta,weights] = RBFparams2dof(k,ndof)
    num_inputs = 3; % 1 = e_q, 3 = [e_q,e_px,e_py]
    dim_inputs = ndof;
