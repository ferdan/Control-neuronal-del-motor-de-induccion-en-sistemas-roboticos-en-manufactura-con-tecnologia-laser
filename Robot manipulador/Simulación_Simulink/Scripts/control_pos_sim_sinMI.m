...
for tr_sel = 5
    ...
    %[~,T0,P] = robot_cinematica.cin_dir(zeros(6,1));
    %P(1,1,6) = P(1,1,6);
    %%%%%%R = T0(1:3,1:3,6);
    
    %r = P(:,1,6).*ones(3,ceil(tsim/tstep));
    %dr = zeros(3,ceil(tsim/tstep));
    %ddr = zeros(3,ceil(tsim/tstep));
    %[q0,dq0,ddq0] = robot_cinematica.cin_inv_6dof(r(:,1),dr(:,1),ddr(:,1),R);
    ...
    robot_din = struct('D',zeros(n,n),'C',zeros(n,n),'G',zeros(n,1),'g',9.81);
    bus_robot_din_info = Simulink.Bus.createObject(robot_din);
    robot_din_bus = evalin('base', bus_robot_din_info.busName);
    
    if n == 3
        simname = 'control_pos_3dof_sinMI';
    elseif n == 6
        simname = 'control_pos_6dof_sinMI';
    end
