function NN = setup_RBF_nets(ndof,kmeans_data,unc_sel)
    ...
    if ndof == 2
        [num_inputs,dim_inputs,delta,weights] = RBFparams2dof(k,ndof);
    elseif ndof == 3
        [num_inputs,dim_inputs,delta,weights] = RBFparams3dof(k,ndof);
    elseif ndof == 6
        [num_inputs,dim_inputs,delta,weights] = RBFparams6dof(k,ndof,unc_sel);
    end

    NN = NN_RBF(num_inputs,dim_inputs,delta,weights,k_params);
end

function [num_inputs,dim_inputs,delta,weights] = RBFparams6dof(k,ndof,unc_sel)

    num_inputs = 4; % 1 = e_q, 4 = [e_q,e_px,e_py,e_pz]
    dim_inputs = ndof;

    deltaWc = 1;
    weightsWc = 18*ones(1,k*dim_inputs);
    ...
    deltaPc = 1;
    weightsPc = 18*ones(1,k*dim_inputs);
...
function [num_inputs,dim_inputs,delta,weights] = RBFparams3dof(k,ndof)
    num_inputs = 4; % 1 = e_q, 4 = [e_q,e_px,e_py,e_pz]
    dim_inputs = ndof;

    deltaWc = 1e-0;
    weightsWc = 20*ones(1,k*dim_inputs);
    ...
    deltaPc = 1e-0;
    weightsPc = 20*ones(1,k*dim_inputs);
    ...
    % 3dof: initial weights: 120,0.5,500, k = 2, num_inputs = 1
end

function [num_inputs,dim_inputs,delta,weights] = RBFparams2dof(k,ndof)
    num_inputs = 1; % 1 = e_q, 3 = [e_q,e_px,e_py]
    dim_inputs = ndof;

    deltaWc = 1e-3;
    weightsWc = 40*ones(1,k*dim_inputs);
    
    deltaZc = 1e-3;
    weightsZc = 1*ones(1,k*dim_inputs);
    ...
    % 2dof: initial weights: 60,1,500, k = 2, num_inputs = 1,3
end
