function MI_control_wR_resumen_simulacion(str_dir_graficas,str_file_res,MI_motor,controlador,w_Rref,tau_L)
    if str_dir_graficas ~= ""
        file_resultados = fopen(str_file_res,'w');
        imprimir_resultados(file_resultados,MI_motor,controlador,w_Rref,tau_L)
        fclose(file_resultados);
    end
    imprimir_resultados(1,MI_motor,controlador,w_Rref,tau_L)
end

function imprimir_resultados(fileID,MI_motor,controlador,w_Rref,tau_L)
    ...
    printPerfil(fileID,w_Rref,MI_motor.w_Rnom,"[N m]")
